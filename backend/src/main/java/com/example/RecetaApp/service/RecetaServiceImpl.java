...
public class RecetaServiceImpl implements RecetaService {
    ...
    public Receta createReceta(Receta receta, Usuario usuario) {
        ...
        // Validar y establecer valores por defecto para los nuevos campos
        if (receta.getTipoCocina() == null || receta.getTipoCocina().trim().isEmpty()) {
            receta.setTipoCocina("Sin especificar");
        }

        if (receta.getPaisOrigen() == null || receta.getPaisOrigen().trim().isEmpty()) {
            receta.setPaisOrigen("Sin especificar");
        }

        if (receta.getPorcion() == null || receta.getPorcion() < 1) {
            receta.setPorcion(1);
        }
        ...
        try {
            return recetaRepository.findById(id)
                .map(existingReceta -> {
                    ...
                    existingReceta.setTipoCocina(receta.getTipoCocina() != null ? 
                        receta.getTipoCocina() : "Sin especificar");
                    
                    existingReceta.setPaisOrigen(receta.getPaisOrigen() != null ? 
                        receta.getPaisOrigen() : "Sin especificar");
                    
                    existingReceta.setPorcion(receta.getPorcion() != null && receta.getPorcion() > 0 ? 
                        receta.getPorcion() : 1);
                    
                    existingReceta.setNotasExtra(receta.getNotasExtra());
